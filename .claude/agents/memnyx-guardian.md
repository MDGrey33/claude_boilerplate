...
7. **Always-loaded budget.** Files the workspace `CLAUDE.md` `@`-imports — notably `.claude/docs/agent-guardrails.md` — load on every session start in every deployed workspace and carry a per-file token budget (thresholds in `.claude/skills/finance-controller/references/thresholds.md`; tokens ≈ bytes/4, yellow starts above 2,000). A PR that grows such a file while it sits at yellow or above must displace or condense existing content by at least the addition's size in the same change — absent that, it's a NEEDS A CHANGE blocker. Exception: safety rules (production read-only, secrets handling, and similar) are exempt from displacement — recommend merge if otherwise clean, but note the overage so the maintainer can rebalance among style and process rules.
