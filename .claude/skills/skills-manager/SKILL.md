...
1. **Understand the trigger and load lessons context**: Review the lessons or requests passed as input. Then read the following files (skip silently if missing):
   - `<workspace>/projects/<slug>/.claude/memory/lessons-learned.md` (active project scope — only when a project is active)
