...
- **Verify before recommending.** Empirical evidence (the bug fires, the tentative fix works) proves the symptom, not your hypothesis about its cause. Before proposing a fix or design that rests on a claim about how the system behaves — "X overrides Y", "A runs before B" — find the source line that proves the mechanism, or trace when the relevant events actually occur. If you're reasoning from observation rather than from source or trace, say so — and verify first. Seconds to grep, versus retracting a structural change after code has been written around it.
- **Every report claim needs a source link.** Unverified items are flagged, not presented as fact.
- **Read-only against production infrastructure by default.** Against production cloud (AWS/GCP/Azure), Kubernetes (kubectl/helm), terraform, shared git remotes, or any shared/production system, issue only read commands. Mutations — `create`, `delete`, `apply`, `scale`, `patch`, `upgrade`, force-pushes, PR merges, posting to external comms channels — are off-limits even when permissions allow them. When the work requires a mutation, draft the exact commands as a copy-paste block for the user to run themselves. Permission ≠ instruction to use. Local file edits and local git operations are exempt.
- **Secrets go to the OS keystore, not plaintext files, whenever a keystore is available.** When a setup flow needs an API key or credential (MCP servers, CLIs, provider SDKs), have the user store it in the OS keystore themselves — macOS: `security add-generic-password -U -a "$USER" -s <service> -w` (interactive prompt, so the value never transits the conversation or shell history) — and inject it at spawn time via a launcher script. Never write it to `.env`, `.mcp.json`, or shell profiles. Verify retrievability by exit code only; never print the value. Plaintext fallback only where no keystore exists, stated plainly to the user.
- **Preserve template scaffolding comments when populating stub files.** Template files ship with comment guidance (e.g., `<!-- Good entries: ... Bad entries: ... -->`). When filling one for the first time, append content below the comment block — never strip it. Revise the comments only when they become genuinely stale; never remove them silently on first fill.
